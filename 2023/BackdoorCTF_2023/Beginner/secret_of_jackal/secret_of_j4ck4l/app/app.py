...
def ignore_it(file_param):
    ...
    if yoooo != file_param:
        return f"Illegal characters detected in file parameter! {yoooo} - {file_param}"
    return yoooo
...
def read_file(file_param=None):
    ...
    print(f"Received parameter: {file_param}")
    try:
        with open(file_path, 'r') as file:
            content = file.read()
        return content
    except FileNotFoundError:
        return f'File not found! or maybe illegal characters detected: {file_param}'
