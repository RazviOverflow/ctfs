...
class RSAModuli:
	...
	def print_value(self):
		KEY1 = [0x82,0x63,0xc3,0x8d, 0x35] #FLAG{
		KEY2 = [0xa2,0x43,0xe3,0xad, 0x35] #flag{

		print("Plain Text: ", hex(self.m))
		bytes_str = str(hex(self.m))[2:]
		bytes_chr = list()
		bytes_str = [bytes_str[i:i+2] for i in range(0, len(bytes_str), 2)]
		print("BYTES:", bytes_str)

		for index, byte in enumerate(bytes_str):
			bytes_chr.append(chr(int(byte,16)))
			print("KEY1:", chr(int(byte,16)^KEY1[index%5]))
			print("KEY2:", chr(int(byte,16)^KEY2[index%5]))

		print("BYTES CHR:", bytes_chr)
	


		print("KEY1:", hex(int(bytes_str[0],16)^ord("F")), hex(int(bytes_str[1],16)^ord("L")), hex(int(bytes_str[2],16)^ord("A")), hex(int(bytes_str[3],16)^ord("G")), hex(int(bytes_str[4],16)^ord("{")))
		print("KEY2:", hex(int(bytes_str[0],16)^ord("f")), hex(int(bytes_str[1],16)^ord("l")), hex(int(bytes_str[2],16)^ord("a")), hex(int(bytes_str[3],16)^ord("g")), hex(int(bytes_str[4],16)^ord("{")))
